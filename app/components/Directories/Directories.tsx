...
const directoryData = [
  ...
  return (
    <div>
      {/* A-Z */}
      <div className="bg-[#FDEE21] p-3 flex flex-wrap justify-center gap-2">
        {alphabet.map((l) => (
          <button key={l} className="border px-2 py-1">{l}</button>
        ))}
      </div>
      ...
      <div className="bg-gray-100 py-10 text-center">
        ...
        <div className="relative max-w-6xl mx-auto">
          <button onClick={scrollLeft} className="absolute left-0 top-1/2 -translate-y-1/2 bg-black text-white p-2 rounded-full">
            <FaChevronLeft />
          </button>

          <div ref={sliderRef} className="flex overflow-x-auto gap-6 px-10">
            {categories.map((c, i) => (
          ...
          <button onClick={scrollRight} className="absolute right-0 top-1/2 -translate-y-1/2 bg-black text-white p-2 rounded-full">
            <FaChevronRight />
          </button>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-12 gap-4">
          ...
          <div className="hidden lg:block col-span-2">
            <div ref={leftRef} className="sticky top-24 space-y-4">
              <div className={`${showLeftAds ? "animate-left" : "opacity-0"} bg-white h-[300px] shadow`}>
                <iframe src="https://www.youtube.com/embed/SgdcMWAST9s?autoplay=1&mute=1&loop=1&playlist=SgdcMWAST9s" className="w-full h-full"/>
              </div>
              <div className={`${showLeftAds ? "animate-left" : "opacity-0"} bg-white h-[300px] shadow`} style={{animationDelay:"0.2s"}}>
                <iframe src="https://www.youtube.com/embed/H1fyJQXdfzU?autoplay=1&mute=1&loop=1&playlist=H1fyJQXdfzU" className="w-full h-full"/>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-8">
            <h3 className="text-xl text-center mb-6">Bangladeshi Top Companies</h3>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
              {companies.map((c,i)=>(
                <a key={i} href={c.url} target="_blank">{c.name}</a>
              ))}
            </div>
                ...
                <div key={i}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {section.items.map((item,j)=>(
                      <a key={j} href={item.url}>{item.name}</a>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div className="hidden lg:block col-span-2">
            <div ref={rightRef} className="sticky top-24 space-y-4">
              <div className={`${showRightAds ? "animate-right" : "opacity-0"} bg-white h-[300px] shadow`}>
                <iframe src="https://www.youtube.com/embed/JtXVOHKDpHk?autoplay=1&mute=1&loop=1&playlist=JtXVOHKDpHk" className="w-full h-full"/>
              </div>
              <div className={`${showRightAds ? "animate-right" : "opacity-0"} bg-white h-[300px] shadow`} style={{animationDelay:"0.2s"}}>
                <iframe src="https://www.youtube.com/embed/k_DNgdwZeMg?autoplay=1&mute=1&loop=1&playlist=k_DNgdwZeMg" className="w-full h-full"/>
              </div>
            </div>
          </div>
